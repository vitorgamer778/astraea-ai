...
export function ChatSidebar({ conversations, activeId, email, isPreview, open, onClose, onSelect, onNew, onRename, onDelete }: Props) {
  ...
  const visibleConversations = query.trim() ? conversations.filter((item) => item.title.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())) : conversations;
  ...
  return <>
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-50 flex h-dvh w-[min(18rem,88vw)] shrink-0 -translate-x-full flex-col border-r border-white/[0.07] bg-sidebar/95 shadow-2xl backdrop-blur-xl transition-transform lg:static lg:h-screen lg:w-72 lg:translate-x-0 lg:bg-sidebar lg:shadow-none", open && "translate-x-0")}>
      ...
      <div className="space-y-2 px-3"><Button onClick={onNew} className="w-full justify-start shadow-lg shadow-primary/10"><Plus />New conversation</Button>{searchOpen ? <div className="relative"><Search className="absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Escape") { setSearchOpen(false); setQuery(""); } }} placeholder="Search conversations" className="h-9 pl-9" /></div> : <Button onClick={() => setSearchOpen(true)} variant="ghost" className="w-full justify-start text-muted-foreground"><Search />Search conversations</Button>}</div>
      <nav className="mt-5 space-y-1 px-3 text-sm"><p className="px-2 pb-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground/70">Workspace</p><button className="flex w-full items-center gap-3 rounded-lg bg-sidebar-accent px-2.5 py-2 text-left text-foreground"><MessageSquare className="size-4" />Chat</button>{[[Library,"Knowledge"],[Bot,"Agents"],[FileText,"Projects"]].map(([Icon,label]) => <div key={label as string} aria-disabled="true" className="flex w-full items-center gap-3 rounded-lg px-2.5 py-2 text-muted-foreground/50"><Icon className="size-4" /><span>{label as string}</span><span className="ml-auto text-[9px] uppercase tracking-wider">Soon</span></div>)}</nav>
      <Separator className="my-4" />
      <p className="px-5 pb-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground/70">Recent</p>
      <ScrollArea className="min-h-0 flex-1 px-3"><div className="space-y-1 pb-4">{visibleConversations.map((item) => <div key={item.id} className={cn("group flex items-center rounded-lg pr-1 text-sm text-muted-foreground transition hover:bg-sidebar-accent hover:text-foreground", activeId === item.id && "bg-sidebar-accent text-foreground")}><button onClick={() => onSelect(item.id)} className="flex min-w-0 flex-1 items-center gap-2 px-2.5 py-2 text-left"><MessageSquare className="size-3.5 shrink-0 opacity-60" /><span className="truncate">{item.title}</span></button><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon-sm" aria-label={`Options for ${item.title}`} className="size-7 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 data-[state=open]:opacity-100"><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-40"><DropdownMenuItem onSelect={() => requestRename(item)}><Pencil />Rename</DropdownMenuItem><DropdownMenuItem variant="destructive" onSelect={() => requestDelete(item)}><Trash2 />Delete</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>)}{visibleConversations.length === 0 && <p className="px-3 py-6 text-center text-xs text-muted-foreground">No conversations found.</p>}</div></ScrollArea>
